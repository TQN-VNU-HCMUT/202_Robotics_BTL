...
%   [X,Y,Z] = CYLINDER2(R,D,N) forms the unit cylinder based on the symmetry 
%   axis D  and the generator curve in the vector R. Vector R contains the 
%   radius at equally spaced points along the unit height of the cylinder. 
%   D is a vector that defines the direction of the symmetry axis. 
...
if nargs > 1, pos = args{2}; end
if nargs > 2, d = args{3}; end
if nargs > 3, n = args{4}; end
if nargs > 4, height = args{5}; end
...
Y1      = M(:,2);
Z1      = M(:,3);
X       = reshape(X1,size(x,1),size(x,2)) + pos(1);
Y       = reshape(Y1,size(y,1),size(y,2)) + pos(2);
Z       = reshape(Z1,size(z,1),size(z,2)) + pos(3);
